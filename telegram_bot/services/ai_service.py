...
class AIService:
    ...
    async def generate_relevant_answer_from_kb_context(
        ...
        system_prompt = f"""
        Ты — ИИ-ассистент в чате бухгалтерской компании. Твоя задача — сгенерировать релевантный ответ на вопрос клиента,
        используя информацию из базы знаний.
    ...
    async def generate_chitchat_response(
        ...
        system_prompt = f"""
        Ты — умный ИИ-ассистент Алина в чате бухгалтерского бота Аудит Картель. Твоя задача — дать естественный, контекстуальный ответ на короткое сообщение пользователя.
